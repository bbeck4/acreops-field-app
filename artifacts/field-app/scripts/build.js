...
async function startMetro(expoPublicDomain, expoPublicReplId) {
  ...
  const env = {
    ...
    NODE_OPTIONS: [
      ...
      `--require=${path.join(projectRoot, "scripts", "metro-file-map-compat.cjs")}`,
    ].filter(Boolean).join(" "),
...
function updateManifests(manifests, timestamp, baseUrl, assetsByHash) {
  const updateForPlatform = (platform, manifest) => {
    ...
    const publicProjectPath = baseUrl.replace("https://", "") + basePath;
    manifest.extra.expoClient.hostUri = publicProjectPath;
    manifest.extra.expoGo.debuggerHost = publicProjectPath;
